...
const Login = () => {
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            const {data} = await axios.post(`/api/user/${state}`, {name, email, password}); //sending name etc with api

            if(data.success){
                navigate('/')
                setUser(data.user) // Store logged-in user data in state
                setShowUserLogin(false) // Close login popup/modal
            }else{
  ...
  return (
    //setShowUserLogin(false): after clicking of outside the form, the form will not display anymore
